...
def dte_debug_solution(vs_pid, fn_quickfix, write_first):
    logging.info ('== dte_debug_solution %s' % vars())
    dte = _get_dte(vs_pid)
    if not dte: return
    if _dte_has_csharp_projects(dte):
        dte.Documents.CloseAll()
    ...
    try:
        ...
        # debug is not synchronous so we have to wait
        _dte_wait_for_build (dte)
	time.sleep (5)
        _dte_wait_for_debug (dte)
    except Exception, e:
        logging.exception (e)
        _dte_exception (e)
        _vim_activate ()
        return
	# dte_output (vs_pid, fn_quickfix, 'Output')
    sta = 'debug finished : %d' % dte.Mode
    _vim_status (sta)
    _vim_activate ()

def _dte_wait_for_debug (dte):
    try:
        while dte.Mode == vsIDEModeDebug:
            time.sleep (0.1)
    except:
        pass
...
def dte_task_list (vs_pid, fn_quickfix):
    ...
    for window in dte.Windows:
        if window.Caption.startswith('Task List'):
            task_list = window
    if not task_list:
...
def dte_output (vs_pid, fn_output, window_caption, notify=None):
    ...
    if window_caption == 'Output':
        ...
        owp = window.Object.OutputWindowPanes.Item (u'\u751f\u6210')
        sel = owp.TextDocument.Selection
    else:
        sel = window.Selection
    sel.SelectAll()
    #print sel.Text.encode("gb18030")
    lst_text = str(sel.Text.encode("gb18030")).splitlines()
...
def _dte_project_tree (project):
    ...
    name = str(name.encode("gb18030"))
    #try:
        #name = str(name.encode("gb18030"))
    #except UnicodeEncodeError:
        #pass

    # logging.info(" project: %s", vars())
